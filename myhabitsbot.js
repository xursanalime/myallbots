...
async function createUser(db, userId, firstName, username, startDateStr) {
  const user = await getUser(db, userId);
  if (user)
    return user;
  await db.prepare(
    "INSERT INTO users (id, first_name, username, start_date, notifications_enabled, current_state) VALUES (?, ?, ?, ?, 1, NULL)"
  ).bind(userId, firstName, username, startDateStr).run();
  return await getUser(db, userId);
}
__name(createUser, "createUser");
async function setUserState(db, userId, state) {
  await db.prepare("UPDATE users SET current_state = ? WHERE id = ?").bind(state, userId).run();
}
__name(setUserState, "setUserState");
async function toggleNotifications(db, userId) {
  const user = await getUser(db, userId);
  if (user) {
    const newState = user.notifications_enabled === 1 ? 0 : 1;
    await db.prepare("UPDATE users SET notifications_enabled = ? WHERE id = ?").bind(newState, userId).run();
  }
}
__name(toggleNotifications, "toggleNotifications");
async function getTasks(db, userId) {
  const { results } = await db.prepare("SELECT * FROM user_tasks WHERE user_id = ? AND is_active = 1 ORDER BY id").bind(userId).all();
  return results || [];
}
__name(getTasks, "getTasks");
async function addTask(db, userId, taskName) {
  await db.prepare("INSERT INTO user_tasks (user_id, task_name) VALUES (?, ?)").bind(userId, taskName).run();
}
__name(addTask, "addTask");
async function deactivateTask(db, taskId, userId) {
  await db.prepare("UPDATE user_tasks SET is_active = 0 WHERE id = ? AND user_id = ?").bind(taskId, userId).run();
}
__name(deactivateTask, "deactivateTask");
async function getTodayLog(db, userId, dateStr, dayNumber) {
  let log = await db.prepare("SELECT * FROM daily_logs WHERE user_id = ? AND date = ?").bind(userId, dateStr).first();
  if (!log) {
    const result = await db.prepare(
      "INSERT INTO daily_logs (user_id, day_number, date, is_passed, trick_score, note) VALUES (?, ?, ?, 0, 0, NULL) RETURNING id"
    ).bind(userId, dayNumber, dateStr).first();
    log = await db.prepare("SELECT * FROM daily_logs WHERE id = ?").bind(result.id).first();
  }
  const tasks = await getTasks(db, userId);
  if (tasks.length > 0) {
    const existingLogs = await db.prepare("SELECT task_id FROM task_logs WHERE log_id = ?").bind(log.id).all();
    const existingTaskIds = new Set((existingLogs.results || []).map((r) => r.task_id));
    for (const t of tasks) {
      if (!existingTaskIds.has(t.id)) {
        await db.prepare("INSERT INTO task_logs (log_id, task_id, is_done) VALUES (?, ?, 0)").bind(log.id, t.id).run();
      }
    }
  }
  return log;
}
__name(getTodayLog, "getTodayLog");
async function getDayLogByNumber(db, userId, dayNumber) {
  return await db.prepare("SELECT * FROM daily_logs WHERE user_id = ? AND day_number = ?").bind(userId, dayNumber).first();
}
__name(getDayLogByNumber, "getDayLogByNumber");
async function getTaskLogs(db, logId) {
  const { results } = await db.prepare(`
    SELECT tl.id, tl.task_id, tl.is_done, ut.task_name 
    FROM task_logs tl
    JOIN user_tasks ut ON tl.task_id = ut.id
    WHERE tl.log_id = ?
  `).bind(logId).all();
  return results || [];
}
__name(getTaskLogs, "getTaskLogs");
async function toggleTaskLog(db, logId, taskId) {
  const current = await db.prepare("SELECT is_done FROM task_logs WHERE log_id = ? AND task_id = ?").bind(logId, taskId).first();
  if (!current) {
    await db.prepare("INSERT INTO task_logs (log_id, task_id, is_done) VALUES (?, ?, 1)").bind(logId, taskId).run();
  } else {
    const newVal = current.is_done ? 0 : 1;
    await db.prepare("UPDATE task_logs SET is_done = ? WHERE log_id = ? AND task_id = ?").bind(newVal, logId, taskId).run();
  }
}
__name(toggleTaskLog, "toggleTaskLog");
async function updateLogStats(db, logId) {
  const logs = await getTaskLogs(db, logId);
  const total = logs.length;
  if (total === 0)
    return;
  const done = logs.filter((l) => l.is_done === 1).length;
  const isPassed = done / total >= 0.5 ? 1 : 0;
  const trickScore = isPassed ? 0.1 : 0;
  await db.prepare("UPDATE daily_logs SET is_passed = ?, trick_score = ? WHERE id = ?").bind(isPassed, trickScore, logId).run();
}
__name(updateLogStats, "updateLogStats");
async function saveNote(db, logId, noteText) {
  await db.prepare("UPDATE daily_logs SET note = ? WHERE id = ?").bind(noteText, logId).run();
}
__name(saveNote, "saveNote");
async function getStats(db, userId) {
  const logsResult = await db.prepare("SELECT * FROM daily_logs WHERE user_id = ? ORDER BY day_number").bind(userId).all();
  ...
  for (const log of logs) {
    currentTrickScore += log.trick_score || 0;
    if (log.is_passed) {
      streak++;
      completedDays++;
    } else {
  ...
  const taskLogsResult = await db.prepare(`
    SELECT log_id, COUNT(*) as total, SUM(is_done) as done 
    FROM task_logs 
    JOIN daily_logs ON daily_logs.id = task_logs.log_id
    WHERE daily_logs.user_id = ?
    GROUP BY log_id
  `).bind(userId).all();
  for (const row of taskLogsResult.results || []) {
    if (row.total > 0 && row.total === row.done) {
      perfectDays++;
    }
  }
  return { currentTrickScore, streak, completedDays, perfectDays };
}
__name(getStats, "getStats");
async function getLogsPage(db, userId, page = 1, pageSize = 10) {
  const offset = (page - 1) * pageSize;
  const countRes = await db.prepare("SELECT COUNT(*) as count FROM daily_logs WHERE user_id = ?").bind(userId).first();
  const total = countRes.count || 0;
  const totalPages = Math.ceil(total / pageSize) || 1;
  const { results } = await db.prepare("SELECT * FROM daily_logs WHERE user_id = ? ORDER BY day_number DESC LIMIT ? OFFSET ?").bind(userId, pageSize, offset).all();
  return { logs: results || [], totalPages, page };
}
__name(getLogsPage, "getLogsPage");
async function getActiveUsersForReminder(db) {
  const { results } = await db.prepare("SELECT * FROM users WHERE notifications_enabled = 1").all();
  return results || [];
}
__name(getActiveUsersForReminder, "getActiveUsersForReminder");
async function addMotivationVideo(db, fileId, caption, addedBy) {
  await db.prepare("INSERT INTO motivation_videos (file_id, caption, added_by) VALUES (?, ?, ?)").bind(fileId, caption, addedBy).run();
}
__name(addMotivationVideo, "addMotivationVideo");
async function getRandomMotivationVideo(db) {
  return await db.prepare("SELECT * FROM motivation_videos ORDER BY RANDOM() LIMIT 1").first();
}
__name(getRandomMotivationVideo, "getRandomMotivationVideo");
async function getMotivationVideosCount(db) {
  const res = await db.prepare("SELECT COUNT(*) as cnt FROM motivation_videos").first();
  return res.cnt || 0;
}
__name(getMotivationVideosCount, "getMotivationVideosCount");

// src/utils.js
function getCurrentDateString(offsetHours = 5) {
  const now = /* @__PURE__ */ new Date();
  const localTime = now.getTime() + offsetHours * 60 * 60 * 1e3;
...
function formatUzbekDate(dateStr) {
  ...
  return `${parts[2]}-${months[m2]}, ${parts[0]}`;
}
__name(formatUzbekDate, "formatUzbekDate");
function calculateDayNumber(startDateStr, currentDateStr) {
...
__name(calculateDayNumber, "calculateDayNumber");
var motivationQuotes = [
  "Bugun qilgan mehnatingiz ertangi kuningiz poydevoridir. Taslim bo'lmang! \u{1F4AA}",
  "Katta natijalar har kuni bajariladigan kichik qadamlardan boshlanadi. \u{1F680}",
  "O'zgarishlar birdaniga bo'lmaydi, har kuni 1% yaxshilanib boring! \u{1F31F}",
  "Intizom bu - hozirgi xohishingizdan ko'ra, kelajakdagi maqsadingizni ustun qo'yishdir. \u{1F3AF}",
  "Agar charchagan bo'lsangiz, dam oling... lekin taslim bo'lmang! \u{1F525}",
  "Muvaffaqiyatga eltuvchi yagona yo'l bu - to'xtamaslikdir. \u{1F6B6}\u200D\u2642\uFE0F"
];
function getRandomMotivation() {
  const i = Math.floor(Math.random() * motivationQuotes.length);
  return motivationQuotes[i];
}
__name(getRandomMotivation, "getRandomMotivation");

// src/handlers.js
function getMainMenuKeyboard() {
  return new Keyboard().text("\u{1F4CB} Bugungi kun").text("\u{1F4CA} Statistika").row().text("\u{1F4C6} Jurnal").text("\u{1F3AF} Vazifalarim").row().text("\u{1F4A1} Motivatsiya").text("\u2699\uFE0F Sozlamalar").resized();
}
...
async function handleStart(ctx) {
  ...
  if (!userId)
    return;
  const existingUser = await getUser(ctx.env.DB, userId);
  if (existingUser) {
    let welcomeText = `Salom, <b>${existingUser.first_name || "Foydalanuvchi"}</b>! \u{1F44B}

`;
    welcomeText += `\u{1F3AF} <b>IntizomBot</b> orqali odatlaringizni shakllantirishda davom eting!
`;
    welcomeText += `Boshlangan sana: <b>${formatUzbekDate(existingUser.start_date)}</b>

`;
    welcomeText += `<i>Quyidagi menyu orqali boshqarishingiz mumkin:</i>`;
    await ctx.reply(welcomeText, {
      parse_mode: "HTML",
      reply_markup: getMainMenuKeyboard()
    });
    return await handleToday(ctx, existingUser);
  }
  const todayStr = getCurrentDateString();
  const formattedToday = formatUzbekDate(todayStr);
  let introText = `\u{1F3AF} <b>IntizomBot'ga xush kelibsiz!</b>
`;
  introText += `\u2501\u2501\u2501\u2501\u2501\u2501\u2501\u2501\u2501\u2501\u2501\u2501\u2501\u2501\u2501\u2501\u2501\u2501\u2501\u2501\u2501

`;
  introText += `Ushbu bot qat'iy intizom va kichik maqsadlar orqali kunlik odatlaringizni shakllantirishga yordam beradi.

`;
  introText += `\u26A1 <b>Qanday ishlaydi?</b>
`;
  introText += `\u2022 O'zingiz uchun muhim vazifalarni qo'shasiz (masalan: Kitob o'qish, Sport).
`;
  introText += `\u2022 Har kuni bajarilgan vazifalarni belgilab borasiz.
`;
  introText += `\u2022 Barcha vazifalarning kamida 50% qismi bajarilganda kun muvaffaqiyatli o'tgan hisoblanadi va sizga ball beriladi.

`;
  introText += `O'z ustingizda ishlashni bugundan (<b>${formattedToday}</b>) boshlaymizmi?`;
  const inlineKeyboard = new InlineKeyboard().text(`\u{1F680} Ha, bugundan boshlaymiz!`, `start_challenge:${todayStr}`);
...
async function handleConfirmStart(ctx, startDateStr) {
  const userId = ctx.from?.id;
  if (!userId)
    return;
  const firstName = ctx.from?.first_name || null;
  const username = ctx.from?.username || null;
  const user = await createUser(ctx.env.DB, userId, firstName, username, startDateStr);
  await ctx.answerCallbackQuery({ text: "Muvaffaqiyatli boshlandi! \u{1F525}" });
  let successText = `\u{1F389} <b>Tabriklaymiz! Odatlar shakllantirish safaringiz boshlandi!</b>

`;
  successText += `\u{1F4C5} Boshlanish sanasi: <b>${formatUzbekDate(startDateStr)}</b>

`;
  successText += `Eng avvalo, o'zingiz uchun kunlik vazifalar qo'shib oling. Buning uchun <b>\u{1F3AF} Vazifalarim</b> tugmasini bosing!`;
  await ctx.reply(successText, {
    parse_mode: "HTML",
    reply_markup: getMainMenuKeyboard()
  });
}
__name(handleConfirmStart, "handleConfirmStart");
async function handleTasksMenu(ctx, user) {
  const tasks = await getTasks(ctx.env.DB, user.id);
  let text = `\u{1F3AF} <b>Sizning kunlik vazifalaringiz</b>
`;
  text += `\u2501\u2501\u2501\u2501\u2501\u2501\u2501\u2501\u2501\u2501\u2501\u2501\u2501\u2501\u2501\u2501\u2501\u2501\u2501\u2501\u2501

`;
  if (tasks.length === 0) {
    text += `<i>Hozircha vazifalar qo'shilmagan.</i>

`;
  } else {
    tasks.forEach((t, index) => {
      text += `${index + 1}. <b>${t.task_name}</b>
`;
    });
    text += `
`;
  }
  text += `Vazifalar qo'shish yoki o'chirish uchun pastdagi tugmalardan foydalaning.`;
  const kb = new InlineKeyboard().text("\u2795 Qo'shish", "add_task_prompt");
  if (tasks.length > 0) {
    kb.text("\u274C O'chirish", "delete_task_prompt");
  }
  await ctx.reply(text, { parse_mode: "HTML", reply_markup: kb });
}
__name(handleTasksMenu, "handleTasksMenu");
async function handleTaskPrompt(ctx, user, action) {
  if (action === "add") {
    await setUserState(ctx.env.DB, user.id, "waiting_for_task_name");
    await ctx.reply(`\u270D\uFE0F <b>Yangi vazifa nomini yozing:</b>
<i>(Masalan: 30 daqiqa kitob o'qish)</i>

Bekor qilish uchun /cancel deb yozing.`, { parse_mode: "HTML" });
  } else if (action === "delete") {
    const tasks = await getTasks(ctx.env.DB, user.id);
    if (tasks.length === 0)
      return ctx.reply("Sizda o'chirish uchun vazifalar yo'q.");
    let text = `\u{1F5D1} <b>Qaysi vazifani o'chirmoqchisiz?</b>
Raqamini yuboring:

`;
    tasks.forEach((t, i) => {
      text += `${i + 1}. ${t.task_name}
`;
    });
    await setUserState(ctx.env.DB, user.id, "waiting_for_task_delete");
    await ctx.reply(text, { parse_mode: "HTML" });
  }
}
__name(handleTaskPrompt, "handleTaskPrompt");
async function handleInput(ctx, user) {
  const text = ctx.message?.text?.trim();
  if (!text)
    return;
  if (text.toLowerCase() === "/cancel" || text.toLowerCase() === "bekor qilish") {
    await setUserState(ctx.env.DB, user.id, null);
  ...
  if (state === "waiting_for_task_name") {
    await addTask(ctx.env.DB, user.id, text);
    await setUserState(ctx.env.DB, user.id, null);
    await ctx.reply(`\u2705 <b>"${text}"</b> vazifasi qo'shildi!`, { parse_mode: "HTML" });
    await handleTasksMenu(ctx, user);
  } else if (state === "waiting_for_task_delete") {
    const num = parseInt(text);
    const tasks = await getTasks(ctx.env.DB, user.id);
    if (!isNaN(num) && num > 0 && num <= tasks.length) {
      const task = tasks[num - 1];
      await deactivateTask(ctx.env.DB, task.id, user.id);
      await setUserState(ctx.env.DB, user.id, null);
      await ctx.reply(`\u{1F5D1} <b>"${task.task_name}"</b> o'chirildi.`, { parse_mode: "HTML" });
      await handleTasksMenu(ctx, user);
    } else {
      await ctx.reply("Iltimos, ro'yxatdagi to'g'ri raqamni kiriting yozing yoki /cancel deng.");
    }
  } else if (state.startsWith("waiting_for_note:")) {
    const dayNumber = parseInt(state.split(":")[1]);
    const log = await getDayLogByNumber(ctx.env.DB, user.id, dayNumber);
    if (log) {
      await saveNote(ctx.env.DB, log.id, text.substring(0, 500));
      await setUserState(ctx.env.DB, user.id, null);
      await ctx.reply(`\u2705 Qaydingiz saqlandi!`);
      await handleToday(ctx, user);
    }
  }
}
__name(handleInput, "handleInput");
async function handleToday(ctx, user) {
  const dateStr = getCurrentDateString(user.timezone_offset);
  const dayNumber = calculateDayNumber(user.start_date, dateStr);
  const log = await getTodayLog(ctx.env.DB, user.id, dateStr, dayNumber);
  const taskLogs = await getTaskLogs(ctx.env.DB, log.id);
  let text = `\u{1F4CB} <b>Bugungi kun vazifalari (${dayNumber}-kun)</b>
`;
  text += `Sana: <b>${formatUzbekDate(dateStr)}</b>
`;
  text += `\u2501\u2501\u2501\u2501\u2501\u2501\u2501\u2501\u2501\u2501\u2501\u2501\u2501\u2501\u2501\u2501\u2501\u2501\u2501\u2501\u2501

`;
  if (taskLogs.length === 0) {
    text += `<i>Sizda hozircha vazifalar yo'q. "\u{1F3AF} Vazifalarim" menyusi orqali qo'shing.</i>`;
    return ctx.reply(text, { parse_mode: "HTML", reply_markup: getMainMenuKeyboard() });
  }
  const kb = new InlineKeyboard();
  taskLogs.forEach((tl, index) => {
    const icon = tl.is_done ? "\u2705" : "\u274C";
    text += `${icon} ${tl.task_name}
`;
    kb.text(`${icon} ${tl.task_name.substring(0, 20)}`, `toggle_task:${log.id}:${tl.task_id}`);
    if ((index + 1) % 2 === 0) {
      kb.row();
    }
  });
  if (taskLogs.length % 2 !== 0) {
    kb.row();
  }
  text += `
<i>Bajarilgan vazifalarni pastdagi tugmalar orqali belgilang.</i>`;
  if (log.note) {
    text += `

\u{1F4DD} <b>Qayd:</b> <i>${log.note}</i>`;
  }
  kb.text("\u{1F4DD} Qayd yozish", `add_note:${dayNumber}`);
  kb.text("\u{1F504} Yangilash", "refresh_today");
...
async function handleToggleTask(ctx, logIdStr, taskIdStr) {
  const logId = parseInt(logIdStr);
  const taskId = parseInt(taskIdStr);
  await toggleTaskLog(ctx.env.DB, logId, taskId);
  await updateLogStats(ctx.env.DB, logId);
  await ctx.answerCallbackQuery();
  const user = await getUser(ctx.env.DB, ctx.from.id);
  if (user)
    await handleToday(ctx, user);
}
__name(handleToggleTask, "handleToggleTask");
async function handleStats(ctx, user) {
  const stats = await getStats(ctx.env.DB, user.id);
  const totalDays = calculateDayNumber(user.start_date, getCurrentDateString(user.timezone_offset));
  let text = `\u{1F4CA} <b>Statistika va Progress</b>
`;
  text += `\u2501\u2501\u2501\u2501\u2501\u2501\u2501\u2501\u2501\u2501\u2501\u2501\u2501\u2501\u2501\u2501\u2501\u2501\u2501\u2501\u2501

`;
  text += `\u{1F3AF} <b>Umumiy ball:</b> <code>${stats.currentTrickScore.toFixed(1)}</code>
`;
  text += `\u{1F4C6} <b>O'tgan kunlar:</b> ${totalDays} kun

`;
  text += `\u{1F525} <b>Uzluksiz ketma-ketlik (Streak):</b> ${stats.streak} kun
`;
  text += `\u2705 <b>Muvaffaqiyatli kunlar:</b> ${stats.completedDays} kun
`;
  text += `\u{1F31F} <b>To'liq (100%) bajarilgan kunlar:</b> ${stats.perfectDays} kun

`;
  text += `<i>\xABKichik intizomlar takrorlansa, ulkan natijalarga olib keladi!\xBB</i>`;
  const kb = new InlineKeyboard().text("\u{1F504} Yangilash", "refresh_stats");
  if (ctx.callbackQuery) {
    try {
      await ctx.editMessageText(text, { parse_mode: "HTML", reply_markup: kb });
    } catch (e) {
...
async function handleJournal(ctx, user, page = 1) {
  const { logs, totalPages } = await getLogsPage(ctx.env.DB, user.id, page, 10);
  let text = `\u{1F4C6} <b>Intizom Jurnali</b>
`;
  text += `\u2501\u2501\u2501\u2501\u2501\u2501\u2501\u2501\u2501\u2501\u2501\u2501\u2501\u2501\u2501\u2501\u2501\u2501\u2501\u2501\u2501
`;
...
`;
  for (const l of logs) {
    const status = l.is_passed ? "\u2705" : "\u274C";
    text += `<b>${l.day_number}-kun</b> (${l.date.substring(5)}) | Ball: ${l.trick_score.toFixed(1)} | ${status}
`;
    if (l.note)
      text += `  \u2514 \u{1F4DD} <i>${l.note}</i>
`;
    text += `
`;
  }
  ...
  const nav = [];
  if (page > 1)
    nav.push(InlineKeyboard.text("\u2B05\uFE0F Oldingi", `journal_page:${page - 1}`));
  if (page < totalPages)
    nav.push(InlineKeyboard.text("Keyingi \u27A1\uFE0F", `journal_page:${page + 1}`));
  if (nav.length > 0)
    kb.row(...nav);
  if (ctx.callbackQuery) {
    try {
...
async function handleSettings(ctx, user) {
  const notifStatus = user.notifications_enabled === 1 ? "\u{1F514} Yoqilgan" : "\u{1F515} O'chirilgan";
  let text = `\u2699\uFE0F <b>Sozlamalar</b>
`;
  text += `\u2501\u2501\u2501\u2501\u2501\u2501\u2501\u2501\u2501\u2501\u2501\u2501\u2501\u2501\u2501\u2501\u2501\u2501\u2501\u2501\u2501

`;
  text += `\u{1F464} <b>Foydalanuvchi:</b> ${user.first_name || "Foydalanuvchi"}
`;
  text += `\u{1F4C5} <b>Boshlangan sana:</b> ${user.start_date}
`;
  text += `\u23F0 <b>Eslatmalar:</b> ${notifStatus}
`;
  const kb = new InlineKeyboard().text(user.notifications_enabled === 1 ? "\u{1F515} Eslatmalarni o'chirish" : "\u{1F514} Eslatmalarni yoqish", "toggle_notifications");
  if (ctx.callbackQuery) {
    try {
      await ctx.editMessageText(text, { parse_mode: "HTML", reply_markup: kb });
    } catch (e) {
...
async function sendMotivation(ctx) {
  const video = await getRandomMotivationVideo(ctx.env.DB);
  const quote = getRandomMotivation();
  const kb = new InlineKeyboard().text("\u{1F3AC} Yana bitta", "more_motivation");
  if (video) {
    const cap = video.caption ? `\u{1F525} <b>Motivatsiya:</b>
<i>"${video.caption}"</i>

${quote}` : quote;
    await ctx.replyWithVideo(video.file_id, { caption: cap, parse_mode: "HTML", reply_markup: kb });
  } else {
    await ctx.reply(`\u{1F525} <b>KUNLIK MOTIVATSIYA:</b>

${quote}`, { parse_mode: "HTML", reply_markup: kb });
  }
}
__name(sendMotivation, "sendMotivation");
...
function createBot(env) {
  ...
  bot.hears("\u{1F4CB} Bugungi kun", async (ctx) => {
    console.log("HEARS: Bugungi kun from", ctx.from.id);
    const user = await getUser(ctx.env.DB, ctx.from.id);
    if (user)
      await handleToday(ctx, user);
    else
  ...
  bot.hears("\u{1F4CA} Statistika", async (ctx) => {
    console.log("HEARS: Statistika from", ctx.from.id);
    const user = await getUser(ctx.env.DB, ctx.from.id);
    if (user)
  ...
  bot.hears("\u{1F4C6} Jurnal", async (ctx) => {
    console.log("HEARS: Jurnal from", ctx.from.id);
    const user = await getUser(ctx.env.DB, ctx.from.id);
    if (user)
      await handleJournal(ctx, user, 1);
    else
      await handleStart(ctx);
  });
  bot.hears("\u{1F3AF} Vazifalarim", async (ctx) => {
    console.log("HEARS: Vazifalarim from", ctx.from.id);
    const user = await getUser(ctx.env.DB, ctx.from.id);
    if (user)
      await handleTasksMenu(ctx, user);
    else
  ...
  bot.hears("\u{1F4A1} Motivatsiya", async (ctx) => {
    console.log("HEARS: Motivatsiya from", ctx.from.id);
    await sendMotivation(ctx);
  });
  bot.hears("\u2699\uFE0F Sozlamalar", async (ctx) => {
    console.log("HEARS: Sozlamalar from", ctx.from.id);
    const user = await getUser(ctx.env.DB, ctx.from.id);
    if (user)
  ...
  bot.on("callback_query:data", async (ctx) => {
    ...
    if (data.startsWith("start_challenge:")) {
      ...
      return handleConfirmStart(ctx, dateStr);
    }
    if (!user) {
      await ctx.answerCallbackQuery({ text: "Iltimos, avval /start ni bosing!", show_alert: true });
      return;
    }
    if (data === "add_task_prompt") {
      await ctx.answerCallbackQuery();
      return handleTaskPrompt(ctx, user, "add");
    }
    if (data === "delete_task_prompt") {
      await ctx.answerCallbackQuery();
      return handleTaskPrompt(ctx, user, "delete");
    }
    if (data.startsWith("toggle_task:")) {
      const parts = data.split(":");
      return handleToggleTask(ctx, parts[1], parts[2]);
    }
    if (data.startsWith("add_note:")) {
      const dayNum = data.split(":")[1];
      await ctx.answerCallbackQuery();
      await ctx.env.DB.prepare("UPDATE users SET current_state = ? WHERE id = ?").bind(`waiting_for_note:${dayNum}`, user.id).run();
      return ctx.reply("\u{1F4DD} Qisqa qaydingizni yozib yuboring (Bekor qilish uchun /cancel):");
    }
    if (data === "refresh_today") {
      await ctx.answerCallbackQuery({ text: "Yangilandi" });
      return handleToday(ctx, user);
    }
    if (data === "refresh_stats") {
      await ctx.answerCallbackQuery({ text: "Yangilandi" });
      return handleStats(ctx, user);
    }
    if (data.startsWith("journal_page:")) {
      await ctx.answerCallbackQuery();
      return handleJournal(ctx, user, parseInt(data.split(":")[1]));
    }
    if (data === "toggle_notifications") {
      await toggleNotifications(ctx.env.DB, user.id);
      await ctx.answerCallbackQuery();
      const updated = await getUser(ctx.env.DB, user.id);
      return handleSettings(ctx, updated);
    }
    if (data === "more_motivation") {
      await ctx.answerCallbackQuery();
      return sendMotivation(ctx);
    }
    await ctx.answerCallbackQuery();
  });
  bot.on(["message:video", "message:video_note"], async (ctx) => {
    ...
    if (fileId) {
      await addMotivationVideo(ctx.env.DB, fileId, ctx.message.caption || null, ctx.from.id);
      const c = await getMotivationVideosCount(ctx.env.DB);
      await ctx.reply(`\u2705 Video saqlandi! Jami videolar: ${c}`);
    }
  });
  bot.on("message:text", async (ctx) => {
    console.log("TEXT:", ctx.message.text, "from", ctx.from.id);
    const user = await getUser(ctx.env.DB, ctx.from.id);
    if (user && user.current_state) {
      return handleInput(ctx, user);
    }
  });
  return bot;
}
...
async function handleCron(event, env) {
  const bot = new Bot(env.BOT_TOKEN);
  const users = await getActiveUsersForReminder(env.DB);
  const isMorning = (/* @__PURE__ */ new Date()).getUTCHours() < 10;
  for (const user of users) {
    try {
      const dateStr = getCurrentDateString(user.timezone_offset);
      const dayNum = calculateDayNumber(user.start_date, dateStr);
      const log = await getTodayLog(env.DB, user.id, dateStr, dayNum);
      const tasks = await getTaskLogs(env.DB, log.id);
      if (isMorning) {
        let msg = `\u{1F305} <b>Xayrli tong, ${user.first_name || "Foydalanuvchi"}!</b>

`;
        msg += `Bugun <b>${dayNum}-kun</b>. O'z ustingizda ishlashda davom eting!
`;
        if (tasks.length === 0) {
          msg += `<i>Hali vazifalar qo'shmapsiz. Vazifalarim menyusidan qo'shishingiz mumkin.</i>`;
        } else {
          msg += `Sizda bugun ${tasks.length} ta vazifa bor. Kamida yarmini bajarishga harakat qiling!`;
        }
        await bot.api.sendMessage(user.id, msg, { parse_mode: "HTML" });
      } else {
        if (tasks.length === 0)
          continue;
        const done = tasks.filter((t) => t.is_done).length;
        if (done >= tasks.length / 2) {
          await bot.api.sendMessage(user.id, `\u2705 <b>Ajoyib!</b> Bugun ${done}/${tasks.length} vazifa bajarildi va kun muvaffaqiyatli o'tdi! \u{1F389}`, { parse_mode: "HTML" });
        } else {
          await bot.api.sendMessage(user.id, `\u{1F319} <b>Kechki xulosa:</b> Hozircha ${done}/${tasks.length} vazifa bajarildi. Agar ulgursangiz, yakunlab qo'ying!`, { parse_mode: "HTML" });
        }
      }
    } catch (e) {
      console.error(e);
...
var src_default = {
  async fetch(request, env, ctx) {
    ...
    if (url.pathname === "/" || url.pathname === "/health") {
      return new Response(JSON.stringify({ status: "online", bot: "IntizomBot" }), { headers: { "Content-Type": "application/json" } });
    }
    if (url.pathname === "/set-webhook") {
      const t = url.searchParams.get("url") || `${url.origin}/webhook`;
      const res = await fetch(`https://api.telegram.org/bot${env.BOT_TOKEN}/setWebhook?url=${encodeURIComponent(t)}&max_connections=1&drop_pending_updates=true`);
      return new Response(JSON.stringify(await res.json(), null, 2), { headers: { "Content-Type": "application/json" } });
    }
    if (request.method === "POST") {
      try {
        console.log("POST received at:", url.pathname);
